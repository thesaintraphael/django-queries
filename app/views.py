...
def search(request):
    ...
    if "postal_code" in params:
        ...
        user_filter.add(Q(postal_code=value), Q.OR)
...
def q_f_together(request):
    ...
    people = Person.objects.filter(
        Q(first_name=F("last_name")) & ~Q(job__icontains="DEV")
    )
    # First name equals to last name and job not include DEV
    print(people)

    people2 = Person.objects.filter(first_name=F("last_name"))  # this works too :D
...
def count_types(request):
    ...
    pet_types = Pet.objects.values("pet_type").annotate(Count("pet_type"))
...
def concat(request):
    ...
    persons = (
        ...
        .filter(full_name__icontains="nn p")
    ...
    # set custom manager
    persons = (
        Person.objects.annotate_full_name()
        .filter(full_name__icontains="nn p")
        .values_list("full_name")
    )
...
def coalesce(request):
    ...
    persons = Person.objects.annotate(
        goes_by=Coalesce(NullIf("nickname", Value("")), "first_name")
    ).values_list("goes_by")
...
def mixed(request):
    ...
    persons = (
        ...
        .values_list("birth_date__year", flat=True)
        .order_by("-id")
    )
...
def coalesce_prevent(request):
    ...
    aggregated = Author.objects.aggregate(
        combined_age=Coalesce(Sum("age"), Value(0)), combined_age_default=Sum("age")
    )
...
def extract(request):
    ...
    experiments_count = Experiment.objects.filter(
        start_datetime__year=Extract("end_datetime", "year")
    ).count()
