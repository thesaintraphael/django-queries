...
from app_related.models import Order, Product
...
def prefetch_filter(request):
    ...
    for order in qs:
        ...
        products = [
            product.name for product in order.products.filter(price__range=(10, 25))
        ]
...
def using_only(request):

    # by using only we can retrieve 
    # the fields that we need in the current case
    
    products = Product.objects.all().only("price")
    
    orders = Order.objects.select_related("customer__email").all().only("id", "name", "customer__email")
